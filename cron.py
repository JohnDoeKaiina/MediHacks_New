...
def send_twilio_message(info):
    ...
    message = client.messages.create(
        body=msg, from_=twilio_phone_number, to=phone_number
    )
        ...
        try:
            ...
            for row in rows:
                ...
                if medicine_status == "Ongoing":
                    ...
                    if (
                        current_time.hour == medicion_time.hour
                        and current_time.minute == medicion_time.minute
                    ):
                        print(
                            f"Current time {current_time} matches medicion_time {medicion_time} in row: {row}"
                        )
                        send_twilio_message(row)

                    else:
                        print(
                            f"Current time {current_time} does not match medicion_time {medicion_time} in row: {row}"
                        )
